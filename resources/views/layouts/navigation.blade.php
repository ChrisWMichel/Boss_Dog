<header x-data="{
    ...
    <div class="flex">
        <a href="{{ route('home.front') }}" class="block pt-2 pl-5">
            <img src="{{ asset('images/projectLogo-small.png') }}?v=1" alt="Logo" class="h-14" />
        </a>
        ...
        <ul>
            ...
            @if (Auth::check())
                <li x-data="{ open: false }" class="relative">
                    <a @click="open = !open"
                        class="flex items-center justify-between px-3 py-2 cursor-pointer hover:bg-slate-800">
    ...
    <nav class="hidden md:block">
        <ul class="grid items-center grid-flow-col">
            ...
            @if (Auth::check())
                <li x-data="{ open: false }" class="relative">
                    <a @click="open = !open"
                        class="flex items-center pr-5 cursor-pointer py-navbar-item px-navbar-item hover:bg-slate-900">
                        <span class="flex items-center">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 mr-2" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round"
