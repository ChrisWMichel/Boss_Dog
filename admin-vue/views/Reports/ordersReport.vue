  ...
  <div class="flex flex-col gap-4 md:flex-row">
    <div class="w-full md:w-[85%]">
      <BarChart v-if="chartData.labels.length > 0" :data="chartData" />
        <spinner v-else class="my-auto mt-5 text-sm" size="w-32 h-32" text="Loading Orders..." />
      </div>
      <div class="w-full md:w-[15%]">
        ...
        <div class="flex justify-center mt-4">
          <button @click="applyDateFilter" class="px-4 py-2 text-lg text-white bg-indigo-500 rounded hover:bg-indigo-700 hover:text-white">Filter</button>
          </div>
      </div>
  </div>
</template>
  ...
  axiosClient
    ...
    .then((response) => {
      ...
      if (Array.isArray(response.data) && response.data.length > 0) {
        chartData.value = {
          labels: response.data.map((order) => {
            ...
            try {
              ...
              if (!isNaN(date.getTime())) {
                ...
                return order.created_at; // Fallback to the original string
              }
            } catch (e) {
              console.error("Error parsing date:", e);
              return order.created_at || "Unknown date";
            }
          }),
          datasets: [
            {
              label: "Order Total",
              backgroundColor: "#4bc0c0",
              data: response.data.map((order) => Number(order.total)),
            },
          ],
        };
      }
      ...
      .then((response) => {
        ...
        if (Array.isArray(response.data) && response.data.length > 0) {
          ...
          if (response.data[0].created_at) {
            ...
            console.error("Response data missing created_at field:", response.data[0]);
          }
        }
